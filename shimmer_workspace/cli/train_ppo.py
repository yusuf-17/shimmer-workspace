...
def init_policy_from_gw(model, gw_weights):
    ...
    target_layers = pi_linear_layers + [model.policy.action_net]
    #target_layers = []
    ...
    freeze_actor_parameters(model.policy, train_last_two_layers=True)
...
if __name__ == '__main__':
    ...
    '''
    #changed vf_coef from 0.5 to 0.001, mini_batch_size from 3125 to 1024, ent_coef 0.005 to 0.01, lr from 1e-3 to 4e-4, num epochs 16 ->4, normalize_advantages set to True
    run_config = {
        "model": "PPO",
        ...
        "target_kl": 0.005,
        "clip_range_vf": None,
        "critic_warmup_steps": 200_000,
        "init_policy_from_gw": init_gw,
        "vae_run": vae_run,
        #"eval_episodes": 5,
        #"resume_model_path": "iatzsylp/model.zip",
    }
    '''
    ...
    #new_correctly_normalized_gw_with_bigger_vae
    gw_ckpt = "n1pfpmrp"
    ckpt_epoch = "250"  
...
'''
        CriticWarmupCallback(
            warmup_steps=int(run.config.get("critic_warmup_steps", 0)),
            verbose=1,
        ),
        '''